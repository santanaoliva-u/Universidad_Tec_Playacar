...
// src/model/format/reminder-src-plugin.ts
...
// src/model/format/reminder-src-plugin.ts
var srcSetting = new class srcSetting {
  get dateTrigger() {
  ...
  get(key, defaultValue) {
    ...
    const plugin = plugins["obsidian-src"];
    if (!plugin) {
      return defaultValue;
    }
...
var srcDateTimeFormat = _srcDateTimeFormat;
srcDateTimeFormat.instance = new _srcDateTimeFormat(srcSetting);
var srcReminderModel = class {
  constructor(title, time) {
    this.title = title;
    this.time = time;
  }
  static parse(line, strictDateFormat) {
    const splitted = srcDateTimeFormat.instance.split(line, strictDateFormat);
    if (splitted.time == null) {
      return null;
    }
    return new srcReminderModel(splitted.title, splitted.time);
  }
  getTitle() {
  ...
  toMarkdown() {
    return `${this.title.trim()} ${srcDateTimeFormat.instance.format(this.time)}`;
  }
};
var _srcReminderFormat = class extends TodoBasedReminderFormat {
  parseReminder(todo) {
    return srcReminderModel.parse(todo.body, this.isStrictDateFormat());
  }
  newReminder(title, time) {
    const parsed = new srcReminderModel(title, time);
    parsed.setTime(time);
    return parsed;
  }
};
var srcReminderFormat = _srcReminderFormat;
srcReminderFormat.instance = new _srcReminderFormat();
...
var srcPluginReminderFormat = new ReminderFormatType("srcPluginReminderFormat", "src plugin format", "@{2021-09-08}", srcReminderFormat.instance, false);
var ReminderFormatTypes = [
  reminderPluginReminderFormat,
  tasksPluginReminderFormat,
  srcPluginReminderFormat
];
...
var Settings = class {
  constructor() {
    ...
    this.settings.newGroup("Reminder Format - src Plugin").addSettings(
      reminderFormatSettings.enablesrcPluginReminderFormat
    );
...
var ReminderFormatSettings = class {
  constructor(settings) {
    ...
    this.enablesrcPluginReminderFormat = this.createUseReminderFormatSetting(srcPluginReminderFormat);
  }
